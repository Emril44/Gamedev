...
public class Skin : ScriptableObject
{
    ...
    [field: SerializeField] public List<string> conditionKeyValues { get; private set; }

    public bool IsAvailable()
    {
        for (int i = 0; i < conditionKeyNames.Count; i++) if (PlayerPrefs.GetString(conditionKeyNames[i]) != conditionKeyValues[i]) return false;
        return true;
    }
}
